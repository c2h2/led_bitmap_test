...
def read_img
  ...
  blocks.times do |b|
    16.times do |y|
      byte_color = 0
      8.times do |x|
        if img.pixel_color(x+ b*8+ 1 ,y+2).green> 39000
          g=1
        else
          g=0
        end
...
class RS485
  ...
  def self.rs485_send_raw str
    crc_data = crc16(str.to_bin).to_padded_hex(4)
    puts "CRC16(sent): #{crc_data}"
    puts "CRC16_2: #{crc16_2(str.to_bin).to_padded_hex(4)}"
    RS485.rs485_send("a5"+ RS485.escape(str+crc_data) +"5a")
  end
...
RS485.rs485_send_raw(("00 00 15 10 01 00 01" +  data ).gsub(/\s+/, ""))
